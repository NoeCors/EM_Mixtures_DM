...
{
  # y are the data
  # k number of the levels of the latent variable components
  ...
  while ((h < it) & (ratio > eps )) {
    ...
    # Estimation of f(z|y): normalize for the denominator
    f.z.y<-f.z.y/matrix(rowSums(f.z.y),numobs,k,byrow=FALSE)
    
    
    ######## M-STEP 
    
    # Estimation of pigreco
...
## Functions for estimating Beta #####

 # Beta: function that needs to be maximized ####
...
# alpha: function that needs to be maximized ####
alpha.f<-function(alpha,y,Beta,f.z.y){
  temp=calc.f.y.z(y,alpha,Beta)
  return(-sum(temp*f.z.y))
}  
 
 ## Function for estimating Alpha #####
 
 # alpha: Gradient function #####
